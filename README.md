...
Генерируемые события:
- `basket:change` - при любом изменении корзины (добавление, удаление, очистка)
...
Свойства:
- `items: HTMLElement[]` - устанавливает готовые DOM-элементы корзины
- `total: number` - устанавливает общую стоимость

Методы класса:
- `setCheckoutEnabled(enabled: boolean): void` - управляет состоянием кнопки оформления
